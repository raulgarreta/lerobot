...
class KochRobotConfig(ManipulatorRobotConfig):
    ...
    leader_arms: dict[str, MotorsBusConfig] = field(
        default_factory=lambda: {
            "main": DynamixelMotorsBusConfig(
                port="/dev/tty.usbmodem58FD0167771",
                motors={
                    # name: (index, model)
    ...
    follower_arms: dict[str, MotorsBusConfig] = field(
        default_factory=lambda: {
            "main": DynamixelMotorsBusConfig(
                port="/dev/tty.usbmodem58FD0169131",
                motors={
                    # name: (index, model)
...
@RobotConfig.register_subclass("so100_with_koch")
@dataclass
class So100WithKochRobotConfig(ManipulatorRobotConfig):
    calibration_dir: str = ".cache/calibration/so100_with_koch"
    # `max_relative_target` limits the magnitude of the relative positional target vector for safety purposes.
    # Set this to a positive scalar to have the same value for all motors, or a list that is the same length as
    # the number of motors in your follower arms.
    max_relative_target: int | None = None
...
@RobotConfig.register_subclass("lekiwi_with_koch")
@dataclass
class LeKiwiWithKochRobotConfig(LeKiwiRobotConfig):
    # `max_relative_target` limits the magnitude of the relative positional target vector for safety purposes.
    # Set this to a positive scalar to have the same value for all motors, or a list that is the same length as
    # the number of motors in your follower arms.
    max_relative_target: int | None = None
    ...
    calibration_dir: str = ".cache/calibration/lekiwi_with_koch"
